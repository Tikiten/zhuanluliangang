...
if __name__ == '__main__':
    ...
    for i in range(opt.begin_epoch, opt.n_epochs + 1):
        if not opt.no_train:
            ...
            print('loss_epoch:{0:0.3f}\t''acc_epoch:{1:0.3f}'.format(loss,acc))
            '''# 更新窗口图像  
            viz.line([loss], [i], win='train_loss', update='append')   
            viz.line([acc], [i], win='train_Acc', update='append')'''
        if not opt.no_val:
    ...
    if opt.test:
        ...
        tester.test(test_loader, model, opt, test_data.class_names)
